...
// the code below in comments is used for debugging

static void print(){
	print_binary(command);
	printf("in_buffer: %s\nprotover_setting: %d\n", information.move, information.setting);
}

int fd;
int end = 0;

static void write_buffer(char *input){
...
int main(){
	fd = open("out", O_WRONLY | O_TRUNC, S_IWUSR);
	while(!end){
		read_input_1();
	}
	close(fd);
